...
public struct Vec2i : IEquatable<Vec2i>
{
    ...
    public static Vec2i operator *(int scalar, Vec2i a) => new(a.X * scalar, a.Y * scalar);
    public static Vec2i operator /(Vec2i a, int scalar) => new(a.X / scalar, a.Y / scalar);
    ...
    /// <summary>
    /// Similar to Distance but faster to compute
    /// </summary>
    public static float Heuristic(Vec2i a, Vec2i b) => Math.Abs(a.X - b.X) + Math.Abs(a.Y - b.Y); 
    ...
    public static explicit operator Vec2i(Vec2 v) => new((int)MathF.Round(v.X), (int)MathF.Round(v.Y));
    public Vec2 ToVec2() => new(X, Y);
    public Vec2 ToVec2Floor() => new((float)MathF.Floor(X), (float)MathF.Floor(Y));
...
{
    public static float Heuristic(this Vec2i a, Vec2i b) => Vec2i.Heuristic(a, b);
}