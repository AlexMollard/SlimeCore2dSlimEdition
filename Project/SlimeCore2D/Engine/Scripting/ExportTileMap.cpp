...
SLIME_EXPORT void __cdecl TileMap_SetTile(void* tileMap, int x, int y, int layer, void* texturePtr, float r, float g, float b, float a)
{
    if (tileMap) ((TileMap*)tileMap)->SetTile(x, y, layer, texturePtr, r, g, b, a);
}
...
SLIME_EXPORT void __cdecl TileMap_Render(void* tileMap)
{
    ...
    if (scene)
    {
        ...
        if (camEntity != NullEntity)
        {
            ...
            // Push TileMap back slightly to ensure it's behind Z=0 entities
            // But keep it close to avoid clipping
